...
declare_id!("BVf2Gmc9zX5HUXGuLjDkTrFYjZmNz7Z2UHohMqQoa5rk");
...
pub mod quidproquo {
    ...
    pub fn make_offer_for_nft(
        ...
        offer.offer_valid_till = offer_valid;
    ...
    pub fn accept(ctx: Context<Accept>, offer_bump:u8, _stick_bump:u8, _data_bump:u8, offer_valid:i64) -> ProgramResult {
        ...
        if offer.offer_expired == true {
            return Err(ProgramError::Custom(0x6));
        }

        let clock = &ctx.accounts.clock;
        if clock.unix_timestamp >= ctx.accounts.offer.offer_valid_till {
            return Err(ProgramError::Custom(0x4));
        }
    ...
    pub fn close_offer_pda(ctx: Context<CloseOfferPDA>, _offer_bump:u8, data_bump:u8, offer_valid:i64) -> ProgramResult {
    ...
    pub fn close_stick_pda(ctx: Context<CloseStickPDA>, stick_bump:u8, data_bump:u8, offer_valid:i64) -> ProgramResult {
...
pub struct Make<'info> {
    #[account(init, payer = offer_maker, seeds = [offer_maker.to_account_info().key.as_ref(), nft_mint.to_account_info().key.as_ref(), offer_valid.to_be_bytes().as_ref()], bump = offer_bump,
    space = 850)]
...
#[derive(Accounts)]
#[instruction(offer_bump:u8, data_bump:u8, offer_valid:i64)]
...
#[derive(Accounts)]
#[instruction(stick_bump:u8, data_bump:u8, offer_valid:i64)]
